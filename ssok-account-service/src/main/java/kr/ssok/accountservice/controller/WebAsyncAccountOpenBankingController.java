...
import kr.ssok.accountservice.async.WebAsyncAccountOpenBankingService;
 ...
 */
@RestController
@RequestMapping("/api/accounts/openbank/async/web")
@RequiredArgsConstructor
public class WebAsyncAccountOpenBankingController {
    private final WebAsyncAccountOpenBankingService asyncAccountOpenBankingService;

    @PostMapping
    public CompletableFuture<ResponseEntity<BaseResponse<List<AllAccountsResponseDto>>>> getOpenBankingAccounts(
            @RequestHeader("X-User-Id") String userId) {

        return asyncAccountOpenBankingService.fetchAllAccountsAsync(Long.parseLong(userId))
