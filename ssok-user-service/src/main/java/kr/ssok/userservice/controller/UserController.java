...
public class UserController {
    ...
    /**
     * 인증코드 확인 API (기존 사용자 확인 포함)
     * 사용자가 입력한 인증코드와 Redis에 저장된 인증코드를 비교하여 유효성을 검증하고,
     * 인증 성공 시 해당 휴대폰 번호로 등록된 기존 사용자가 있는지 확인합니다.
     *
     * @param verificationDto 휴대폰 번호와 인증코드가 포함된 요청 DTO
     * @return 인증코드 검증 결과 및 기존 사용자 정보
     */
    @PostMapping("/phone/verify-with-user-check")
    public ResponseEntity<BaseResponse<PhoneVerificationResponseDto>> verifyCodeWithUserCheck(@RequestBody PhoneVerificationRequestDto verificationDto) {
        PhoneVerificationResponseDto responseDto = userService.verifyCodeWithUserCheck(
    ...
    /**
     * PIN 번호 변경 API
     * @param requestDto PIN 변경 요청 정보 (사용자 ID, 새로운 PIN)
     * @return PIN 번호 변경 결과
     */
    @PatchMapping("/pin")
    ...
    /**
     * PIN 재등록 API (기존 사용자용)
     * 휴대폰 인증이 완료된 기존 사용자가 PIN을 재등록할 때 사용합니다.
     *
